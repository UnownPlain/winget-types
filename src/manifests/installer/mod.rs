...
impl Installer {
    ...
    /// Inherits manifest-level defaults for an installer.
    ///
    /// Default-valued fields are treated as unspecified, as in [`Self::merge_with`].
    /// Each installer switch overrides the corresponding manifest switch instead of
    /// combining their arguments.
    #[expect(
        clippy::cognitive_complexity,
        reason = "The resulting complexity is generated by a macro"
    )]
    #[must_use]
    pub fn inherit_from(mut self, manifest: &InstallerManifest) -> Self {
        macro_rules! inherit_keys {
            ($($field:ident),*, [$($switch:ident),* $(,)?] $(,)?) => {
                // Exhaustive patterns require new fields to be considered for inheritance.
                ...
                $(
                    if <_ as PartialEq>::eq(&self.$field, &Default::default()) {
                        self.$field.clone_from($field);
                    }
                )*
                $(
                    if self.switches.$switch.is_none() {
                        self.switches.$switch.clone_from($switch);
                    }
                )*
            };
        }
...
mod tests {
    ...
    fn inherit_all_manifest_properties() {
        ...
        "#};
        let manifest: InstallerManifest = serde_yaml::from_str(&alloc::format!(
            "PackageIdentifier: Test.Package\nPackageVersion: '1.0'\nInstallers: []\n{properties}"
        ))
        .unwrap();
        let installer = Installer::default();
        let identity = alloc::format!(
            "Architecture: neutral\nInstallerUrl: {}\nInstallerSha256: {}\n",
            installer.url,
            installer.sha_256,
        );
        let expected: Installer =
            serde_yaml::from_str(&alloc::format!("{identity}{properties}")).unwrap();

        assert_eq!(installer.inherit_from(&manifest), expected);
    }

    #[test]
    fn inherit_preserves_installer_overrides() {
        let installer = Installer {
            minimum_os_version: Some("10.0.22000.0".parse().unwrap()),
            scope: Some(super::Scope::User),
            file_extensions: ["custom".parse().unwrap()].into(),
        ...
        let manifest = InstallerManifest {
            minimum_os_version: Some("10.0.19041.0".parse().unwrap()),
            scope: Some(super::Scope::Machine),
            file_extensions: ["root".parse().unwrap()].into(),
            switches: Switches::builder()
        ...
        let mut expected = installer.clone();
        expected.switches.log.clone_from(&manifest.switches.log);
        assert_eq!(installer.inherit_from(&manifest), expected);
    }

    #[test]
    fn merge_preserves_authentication() {
        let previous = Installer {
            authentication: Some(super::Authentication::default()),
            ..Installer::default()
        };
        assert_eq!(Installer::default().merge_with(previous.clone()), previous);
        let detected = Installer {
            authentication: Some(super::Authentication {
                r#type: super::authentication::AuthenticationType::MicrosoftEntraId,
                ..super::Authentication::default()
            }),
            ..Installer::default()
        };
        assert_eq!(detected.clone().merge_with(previous), detected);
    }

    #[test]
    fn optimize_authentication() {
        let authentication = super::Authentication::default();
        ...
        manifest.optimize();
        assert_eq!(manifest.authentication, Some(authentication));
        assert!(
            manifest
                .installers
                .iter()
                .all(|installer| installer.authentication.is_none())
        );
    }
